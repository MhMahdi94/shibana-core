...
@section('content')
    <div class="panel">
        <div class="mb-5 flex items-center justify-between">
            <h5 class="text-lg font-semibold dark:text-white-light">Admins Table</h5>
            <a class="font-semibold hover:text-gray-400 dark:text-gray-400 dark:hover:text-gray-600"
                href="{{ route('admin.admins.admins_create') }}">
            ...
            <table>
                ...
                <tbody>
                    ...
                    @foreach ($admins as $admin)
                        <tr>
                            <td class="whitespace-nowrap">{{ $admin->name }}
                            ...
                            <td {{-- class="text-center whitespace-nowrap"
                                ...
                                <label class="w-12 h-6 relative">
                                    <input type="checkbox" {{ $admin->status == 1 ? 'checked' : '' }} 
                                        class="admin_status custom_switch absolute w-full h-full opacity-0 z-10 cursor-pointer peer"
                                        id="admin_status_{{ $admin->id }}" />
                                    <span for="admin_status"
                            ...
                            <td class="text-center">
                                ...
                                <a href="{{ route('admin.admins.admins_edit', $admin->id) }}" type="button"
                                    x-tooltip="Edit">
                                ...
                                <button type="button" class="delete-btn" x-tooltip="Delete"
                                    data-route="{{ route('admin.admins.admins_destroy', $admin->id) }}"
                                    @click="showAlert({{ $admin->id }})">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
    ...
    <script>
        async function showAlert(id, token) {
            ...
            new window.Swal({
                icon: 'question',
                title: 'Delete Admin',
                confirmButtonText: 'Delete this admin account?',
