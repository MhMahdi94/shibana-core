...
<div class="panel">
    ...
    <form action="{{ route('admin.resturants.resturants_update',$resturant->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="panel ">
            ...
            <div class="grid grid-cols-2 gap-2">
                <div>
                    ...
                    <input id="name" name="name" type="text" placeholder="Enter Name" value="{{ $resturant->name }}" class="form-input"
                        required />
                </div>
                <div>
                    <label for="owner_name">Enter Owner Name</label>
                    <input id="owner_name" name="owner_name" type="text" placeholder="Enter Owner Name" value="{{ $resturant->owner_name }}"
                        class="form-input" required />
                </div>
                <div>
                    <label for="email">Enter Email</label>
                    <input id="email" name="email" type="email" placeholder="Enter email" class="form-input" value="{{ $resturant->email }}"
                        required />
                </div>
                <div>
                    <label for="mobile_no">Enter Mobile No</label>
                    <input id="mobile_no" name="mobile_no" type="text" placeholder="Enter Mobile Number" value="{{ $resturant->mobile_no }}"
                        class="form-input" required />
                </div>
                <div>
                    <label for="address">Enter Address</label>
                    <input id="address" name="address" type="text" placeholder="Enter Address" class="form-input" value="{{ $resturant->address }}"
                        required />
                </div>
                <div class="">
                    ...
                    <select id="seachable-select" name="cuisine_id">
                        @foreach ($cuisines as $cuisine)
                            <option value="{{ $cuisine->id }}" {{ $resturant->cuisine_id == $cuisine->id?'selected':''  }}>{{ $cuisine->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label for="longitude">Longitude</label>
                    <input id="longitude" name="longitude" type="text" placeholder="Longitude" class="form-input" value="{{ $resturant->longitude }}"
                        required />
                </div>
                <div>
                    <label for="latitude">Latitude</label>
                    <input id="latitude" name="latitude" type="text" placeholder="Latitude" class="form-input" value="{{ $resturant->latitude }}"
                        required />
                </div>
            </div>
        </div>
