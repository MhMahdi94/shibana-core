...
@section('content')
    <div class="panel">
        ...
        <form action="{{ route('admin.categories.categories_update',$category->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 gap-4">
                <div>
                    <label for="name">Enter Name</label>
                    <input id="name" name="name" type="text" placeholder="Enter Name" value="{{ $category->name }}" class="form-input" required />
                </div>
                <div>
                    <label for="image">Cuisine Image</label>
                    <input id="image" name="image" type="file"  class="form-input" value="{{ $category->image }}"  />
                </div>
            ...
            <div class="grid grid-cols-4 gap-1">
                    ...
                    <a type=""  href="{{ route('admin.categories.categories_index') }}"
